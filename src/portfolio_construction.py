...
def construct_factor_portfolio(
    ...
    """
    # Step 1: Load data, including the exchange code.
    required_cols = ['eom', 'permno', 'crsp_exchcd', 'me', 'ret_exc_lead1m', characteristic]
    char_df = pl.read_parquet(char_data_path, columns=required_cols).drop_nulls(subset=[characteristic, 'me'])

    # Apply standard academic screen for the Value factor
    if characteristic == 'be_me':
        char_df = char_df.filter(pl.col(characteristic) > 0)

    # Calculate breakpoints on NYSE stocks ONLY
    # CRSP exchange code for NYSE is 1.
    ...
    # Step 2: Join NYSE breakpoints to ALL stocks.
    char_df = char_df.join(size_breakpoints, on='eom')

    # Step 3: Determine characteristic breakpoints on non-micro-caps.
    ...
    # Step 5: Calculate capped value-weighted returns.
    portfolio_returns = char_df.group_by(['eom', 'portfolio']).agg(
        ...
        pl.len().alias('n_stocks') # Use .len() instead of deprecated .count()
    ).sort('eom')
    
    # Step 6: Calculate the long-short factor return.
